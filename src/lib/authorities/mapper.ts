import { AuthorityContact, EscalationTierInfo, AuthorityResolutionResult } from "@/lib/types";

export interface MappingRule {
  rule_id: string;
  jurisdiction_keywords: string[];
...
export const PMC_AUTHORITY_MAPPINGS: MappingRule[] = [
...
// Fallback Authority Resolution for unmapped or novel civic complaints
export const FALLBACK_AUTHORITY_RESOLUTION = {
 ...
 * Guaranteed to never fail: automatically falls back to PMC Apex Central Grievance Cell.
 */
export function resolveAuthority(params: ResolveAuthorityInput): AuthorityResolutionResult {
  const jurisdictionInput = (params.jurisdiction || "").trim().toLowerCase();
  const categoryInput = (params.category || "").trim().toLowerCase();
  const deptInput = (params.department || "").trim().toLowerCase();

  let bestRule: MappingRule | null = null;
  let highestScore = 0;

  for (const rule of PMC_AUTHORITY_MAPPINGS) {
    let score = 0;

    // Evaluate jurisdiction match
    for (const jk of rule.jurisdiction_keywords) {
      if (jk && jurisdictionInput.includes(jk.toLowerCase())) {
        score += 5;
        break;
      }
    }
    ...
    for (const ck of rule.category_keywords) {
      if (ck && categoryInput.includes(ck.toLowerCase())) {
        score += 5;
        break;
      }
    }
    ...
    const code = rule.department_code.toLowerCase();
    const name = rule.department_name.toLowerCase();
    if (deptInput && (deptInput.includes(code) || deptInput.includes(name) || code.includes(deptInput))) {
      score += 3;
    }
  ...
  // If score is at least 5 (matched either jurisdiction or category)
  if (bestRule && highestScore >= 5) {
    return {
      jurisdiction: bestRule.jurisdiction_name,
      category: params.category || "Civic Grievance",
      department: bestRule.department_name,
      responsible_authority: bestRule.responsible_authority,
      escalation_chain: bestRule.escalation_chain,
      is_fallback: false,
      mapping_rule_id: bestRule.rule_id,
      resolution_timestamp: new Date().toISOString(),
    };
  }

  // Graceful Fallback
  const fb = FALLBACK_AUTHORITY_RESOLUTION;
  return {
    jurisdiction: params.jurisdiction || fb.jurisdiction_name,
    category: params.category || "Unclassified Civic Issue",
    department: fb.department_name,
    responsible_authority: fb.responsible_authority,
    escalation_chain: fb.escalation_chain,
    is_fallback: true,
