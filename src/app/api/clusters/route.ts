...
export async function GET() {
  ...
  try {
    ...
    const fastApiResponse = await fetch(`${BACKEND_URL}/api/v1/clusters`, {
  ...
  if (!supabase) {
    return NextResponse.json({ total_clusters: 0, clusters: [] });
  }
...
export async function POST(req: NextRequest) {
  ...
  const threshold = typeof body.threshold === "number" ? body.threshold : 0.80;
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (clusterMembers.length > 1) {
        visited.add(g1.id);
        const clusterName = `${clusterMembers[0].category || "Municipal"} Group (${clusterMembers[0].address || "Ward 12"})`;
        suggested.push({
          cluster_name: clusterName,
          category: clusterMembers[0].category || "General",
